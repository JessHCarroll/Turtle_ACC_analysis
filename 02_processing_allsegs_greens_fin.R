...
setwd("C:/Users/xcarrj/Desktop/turtleacc_analysis/greens/lab_accdata/pos1")


files<-list.files()
files<-paste0("C:/Users/xcarrj/Desktop/turtleacc_analysis/greens/lab_accdata/pos1/", files)

tables <- lapply(files, read.csv, header = TRUE)
data_full <- do.call(rbind , tables)


data_short<-data_full[,c(2,3,4,9,10,11)]

acc.data<-data_short

colnames(acc.data)[colnames(acc.data) == "Behavior"] ="behaviour"
colnames(acc.data)[colnames(acc.data) == "Subject"] ="id"
table(acc.data$id)

write.csv(acc.data, "acc.data_IDscombined_p1.csv")
...
write.csv(acc.data, "acc.data_segslabs_p1.csv")
...
my.data<-read.csv("acc.data_segslabs_p1.csv")
table(my.data$segID)
pos="p1"
Hz<-100
...
for(r in 1:length(rolling)){
  ...
  write.csv(empty, paste0("100hzsegmented_accData",bl_len,pos, "seglength_greens.csv"))
...
for(s in 1:length(full_freq)){
  ...
  input_file <- paste0("C:/Users/xcarrj/Desktop/turtleacc_analysis/greens/lab_accdata/pos1/100hzsegmented_accData", full_freq[s],"p1seglength_greens.csv")
  ...
  write.csv(slice_50,paste0("resamp_50hz_", full_freq[s],"_p1seglength_greens.csv"))
  ...
  write.csv(slice_25,paste0("resamp_25hz_", full_freq[s],"_p1seglength_greens.csv"))
  ...
  write.csv(slice_10,paste0("resamp_10hz_", full_freq[s],"_p1seglength_greens.csv"))
  ...
  write.csv(slice_12,paste0("resamp_12hz_", full_freq[s],"_p1seglength_greens.csv"))
  ...
  write.csv(slice_8,paste0("resamp_8hz_", full_freq[s],"_p1seglength_greens.csv"))
  ...
  write.csv(slice_6,paste0("resamp_6hz_", full_freq[s],"_p1seglength_greens.csv"))
  ...
  write.csv(slice_4,paste0("resamp_4hz_", full_freq[s],"_p1seglength_greens.csv"))
  ...
  write.csv(slice_2,paste0("resamp_2hz_", full_freq[s],"_p1seglength_greens.csv"))
...
files<-paste0("C:/Users/xcarrj/Desktop/turtleacc_analysis/greens/lab_accdata/pos1/")
...
for (m in 1:nrow(met_df)){
  ...
  if(met_df$hz_rp[m]==100){
    input_file <- paste0("100hzsegmented_accData", met_df[m,2], pos,"seglength_greens.csv")
  }else{
    input_file <- paste0("resamp_", met_df[m,1], "hz_", met_df[m,2],"_", pos, "seglength_greens.csv")
  }
  
  data<-read.csv(input_file)
  
  if(length(which( colnames(data)=="X.x"))>0){
    data<-data[ , -which(names(data) %in% c("X"))]  
    names(data)[names(data) == 'X.x'] <- 'X'
  }else{
    print("ERROR")
  }
